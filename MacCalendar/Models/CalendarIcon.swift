...
class CalendarIcon: ObservableObject {
    ...
    private func getUpdateInterval() -> (interval: TimeInterval, component: Calendar.Component) {
        ...
        case .custom:
            ...
            if SettingsManager.enableDoubleLine {
                format += SettingsManager.doubleLineTopFormat + SettingsManager.doubleLineBottomFormat
            }
    ...
    private func processCustomFormat(format: String, date: Date) -> String {
        ...
        if hasWeek {
            ...
            let weekValue = calendar.component(.weekOfYear, from: date)
        ...
        if hasLunarMonth {
            let lunarMonth = LunarDateHelper.getLunarMonth(for: date)
